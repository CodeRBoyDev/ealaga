...
function Donation() {
  ...
  useEffect(() => {
    ...
    const [categoryss, setCategory] = useState({ category: "" });
    ...
    const { category } = categoryss;
  
    console.log(category)
  
  
    const filteredCategoryss = getDonation?.filter(filterdonation=> {
  
      console.log(filterdonation)
  
      if (category !== "" && filterdonation?.category !== category) {
        return false
      }
  
        return filterdonation?.category.match(category) 
    });



  var data = {
    columns: ["ID", "Donator Name", "Category", "Quantity", "Date" , "Action"],
    rows: []
  }

  filteredCategoryss?.forEach(donation => {
    data.rows.push([
      donation._id,
      donation.donator_name,
      donation.category,
      donation.quantity,
      moment(donation.donatedAt).format("MMMM DD, YYYY "),
        <>
...
console.log(getsearchUserName)
  ...
  const downloadPDF = () => {
    ...
    axios({
      method: "post",
      url: `/api/donation/downloadPDF`,
      headers: {
        "Content-Type": "application/json",
        'Access-Control-Allow-Origin': '*',
        'Access-Control-Allow-Headers': '*',
      },
      data: JSON.stringify({ downloadPdf: filteredCategoryss, category: category }),
      responseType: 'blob'
    }).then(response => {
    ...
    <div style={contentStyle}>
          ...
          <MUIDataTable
              title={<>
                ...
                <div style={{ display: "flex", flexDirection: "row" }}>
                  ...
                  onClick={onAll} 
                  style={{ marginTop: "1vh", marginRight: "2vh" }}>&nbsp;Reset&nbsp;</Button>
        ...
        <Modal.Body style={{ background: '#ffff' }}>
          ...
          {getSelectDonator.donator_type == "" ?
            ...
            getSelectDonator.donator_type == "exist" ?
              ...
              <Col>
              <TextField fullWidth color="secondary" id="health_problem" value={geteditDonation?.donator_name} onChange={(e) => handleSearchs(e.target.value)}  InputLabelProps={{ shrink: true, required: true }} name="name" label="Donor Name" />
                <InputLabel style={{ color: "red", "font-size": "0.8rem" }}>{error.donator_name}</InputLabel>
              </Col>
      ...
      <Modal size='md' show={viewShow} onHide={handleViewClose} animation={true}>
        ...
        <>
                ...
                <Col>
                  <TextField fullWidth color="secondary" id="health_problem" value={geteditDonation?.donator_name} onChange={(e) => handleSearch(e.target.value)}  InputLabelProps={{ shrink: true, required: true }} name="name" label="Donor Name" />
                  <InputLabel style={{ color: "red", "font-size": "0.8rem" }}>{error.donator_name}</InputLabel>
                </Col>
