...
function User() {
 ...
 const renderThumb = ({ style, ...props }) => {
  ...
  const filteredUsers = getUsers?.filter(filterusers => {
    if (typeof filterusers.account_verified === "string" && typeof filterusers.status === "string"
      ...
      // && filterusers.role?.indexOf(role) !== -1
  ...
  const downloadPDF = () => {
    ...
    axios({
      ...
      data: JSON.stringify({ downloadPdf: filteredUsers, status: statuss, account: accountss, barangay: barangayss  }),
      responseType: 'blob'
    }).then(response => {
  ...
  var data = {
    columns: ["ID", "Full Name", "Barangay", "Gender", "Email", "Activation Status", "Verification Status", "Action"],
    rows: []
  }
  filteredUsers?.forEach(user => {
    data.rows.push([
      ...
      user.email,
      user.status == "active" ? <p style={{color: "green", fontWeight:"bold"}}>Active</p>
      :
      user.status == "inactive" ? <p style={{color: "black", fontWeight:"bold"}}>Not Active</p>
      : <p style={{color: "red", fontWeight:"bold"}}>Restricted</p>
 ...
 const [getNotifyUser, setNotifyUser] = useState({type: "", description: ""});
 const [getNotifyType, setNotifyType] = useState({notify_Type:""});
 const [errors, setErrors] = useState({});

  const onChangeNotify = e => {
    setNotifyUser({ ...getNotifyUser, [e.target.name]: e.target.value })
    setErrors({ ...errors, [e.target.name]: "" });
          }


  const onChangeNotifyType = e => {
    setNotifyType({ notify_Type: e.target.value});
    setErrors({type: "", description: ""});
    setNotifyUser({type: "", description: ""});
        };


          const submitNotifyUSer = () => {

            console.log(getNotifyType.notify_Type)

            const newErrors = {};

                if (!getNotifyType.notify_Type || getNotifyType.notify_Type .trim() === "") {
                  newErrors.notify_Type = "Title is required";
                }

                if (!getNotifyUser.description || getNotifyUser.description.trim() === "") {
                  newErrors.description = "Description is required";
                }
            
              if(getNotifyType.notify_Type == "other_issues"){
                if (!getNotifyUser.type || getNotifyUser.type.trim() === "") {
                  newErrors.type = "Title is required";
                }
              }
            ...
            const newNotification= {
              user_id: getEditUserssss._id,
              type: getNotifyType.notify_Type == "other_issues" ? getNotifyUser.type : getNotifyType.notify_Type,
              description: getNotifyUser.description,
            }
              ...
              headers: {
                ...
                .then(response => {
                  MessagehandleClose();
                                Swal.fire({
                                  title: 'Success!',
                                  text: 'You have successfully send your notification message.',
                                  imageUrl: 'https://res.cloudinary.com/du7wzlg44/image/upload/v1680937281/notificationnnn_izdv0u.gif',
                                  imageWidth: 200,
                                  imageHeight: 200,
                                  imageAlt: 'Custom image',
                                  confirmButtonColor: '#EF3A47',
...
// console.log(getNotifyUser)
  ...
  const MessagehandleShow = () => setMessageShow(true);
  ...
  return (
    ...
    <div style={contentStyle}>
      ...
      <SideBar setSideNavExpanded={setSideNavExpanded} sideNavExpanded={sideNavExpanded} />
        ...
        <Modal size='xl' show={editShow} onHide={handleEditClose} animation={true} className={editShow ? 'blur' : ''}>
          <Modal.Header style={{ background: '#CE3043' }}>
                ...
                <Modal.Body style={{ background: '#ffff' }}>
                  ...
                  <Row>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center' }}>
                      <p style={{ color: 'black', marginRight: '8px' }}>Last Restricted Date:</p>
                      {
                        getEditUserssss?.lastRestricted ?  <p style={{ color: 'red', fontWeight: 'bold' }}>
                        {new Date(getEditUserssss?.lastRestricted).toLocaleDateString()}
                      </p>
                      :
                  ...
                  <Col  className="d-flex justify-content-center">
                        ...
                        {DialysisRequirements?.map(service => (
                        <Form.Check
                        key={service._id}
                        inline1
                        label={<span style={{ maxWidth: "150px", wordBreak: "break-all" }}>{service.description}</span>}
                        name={service._id}
                        type="checkbox"
                        style={{fontSize:"1.1rem"}}
                        value={service._id}
                        defaultChecked={
                          AllRequirements?.requirements && AllRequirements?.requirements.includes(service._id)
                        }
                        onChange={edithandleChangessService}
                      />
                      ))}
                     
                        </div>
                        {/* <div  className="d-flex justify-content-left text-align-left">
                      
                        {TotalAllRequirements?.requirements?.length == "8" ? 
                        <>
                         <p style={{fontSize:"1.1rem", textAlign: "left", textAlign: "left", marginTop: "10px"}}>Status:</p>&nbsp;
                         <p style={{fontSize:"1.1rem", fontWeight: "bold", color:"green", marginTop: "10px"}}>Completed</p>
                         </>
                        : 
                        <>
                         <p style={{fontSize:"1.1rem", textAlign: "left", textAlign: "left", marginTop: "10px"}}>Status:</p>&nbsp;
                         <p style={{fontSize:"1.1rem", fontWeight: "bold",color:"red", marginTop: "10px"}}>In Progress</p>
                         </>
                  ...
                  <BtnWrap2 >
                  <div style={{ display: "flex", flexDirection: "row" }}>
                  <Button outline onClick={() => MessagehandleShow()} color="danger"><i class="fas fa-envelope"></i>&nbsp;Notify User</Button> &nbsp; &nbsp;
        ...
        <Modal size='md' show={Messageshow} onHide={MessagehandleClose} animation={true} className={Messageshow ? 'blur' : ''} >
          <Modal.Header style={{ background: '#CE3043' }}>
            <Modal.Title style={{ color: '#ffff' }}><i class="fas fa-bell"></i>&nbsp;&nbsp;Notify User</Modal.Title>
          </Modal.Header>
          <Modal.Body style={{ background: '#ffff' }}>
          <Row>
         ...
         <Col style={{marginBottom:"20px", marginTop:"20px"}}>
              ...
              <FormControl fullWidth>
                                    ...
                                    <NativeSelect
                                        onChange={onChangeNotifyType}
                                        inputProps={{
                                          name: 'status',
                                          id: 'uncontrolled-native',
                                        }}
                                        color="secondary"
                                        value={getNotifyType.notify_Type}
                                        error={!!errors.notify_Type}
                                        helperText={errors.notify_Type}
                                    >
                                      <option value="" selected disabled> <em>Select Type of Message</em></option>
                                      <option value="invalid_id">Invalid ID</option>
                                      <option value="invalid_information">Invalid Information</option>
                                      <option value="unverified_account">Unverified Account</option>
            ...
            {getNotifyType.notify_Type == "other_issues" &&    
            <TextField
               fullWidth
               color="secondary"
               id="health_problem"
               onChange={onChangeNotify}
               value={getNotifyUser.type} 
               InputLabelProps={{ shrink: true, required: true }}
               name="type"
               placeholder="Write the type of issues here"
              //  label="Title"
               error={!!errors.type}
           ...
           <div  className="panel-activity__status">
              ...
              onChange={onChangeNotify} 
              value={getNotifyUser.description} 
              id="commentfield" placeholder="Write your message here..."  className={`form-control ${errors.description ? "is-invalid" : ""}`}
              ></textarea>
               {errors.description && (
                  <div style={{ color: "red", fontSize: "0.8rem" }}>
                    {errors.description}
              ...
              <div className="actions">
                      ...
                      onClick={() => submitNotifyUSer()}
                      ><i class="fas fa-bell"></i>&nbsp;Send Notification Message</Button>
                      </div>
                  </div>
              </div>
         </Row>
          ...
          <Modal.Footer style={{ background: 'linear-gradient(to bottom, rgba(255,186,186,50%), rgba(255,186,186,0%))' }}>
            <Button style={{ background: '#EF3A47', color: 'white' }} variant="light" onClick={MessagehandleClose}>
              Close
            </Button>
          </Modal.Footer>
        </Modal>
...
.blur {
  ...
  background-color: rgba(0, 0, 0, 0.5); /* Darken the blurred elements */
  pointer-events: none; /* Make the blurred elements non-interactive */
}
