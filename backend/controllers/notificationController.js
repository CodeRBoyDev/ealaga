...
exports.allNotification = catchAsyncErrors(async(req,res,next) => {
  ...
  const schedules = await Schedule.find({ date_schedule: { $gte: now.toDate(), $lt: tomorrow.toDate() } });
  ...
  for (const schedule of schedules) {
    ...
    if (schedule.date_schedule.getDate() === now.date()) {
      description = `We are informing you that you have a schedule today (${moment(schedule.date_schedule).format("MMMM DD, YYYY")}) for ${schedule.category}.`;
      notifToday = true;
    } else if (schedule.date_schedule.getDate() === tomorrow.date()) {
      description = `We are informing you that you have a schedule tomorrow (${moment(schedule.date_schedule).format("MMMM DD, YYYY")}) for ${schedule.category}.`;
      notifTomorrow = true;
    }
    ...
    if (!schedule.notif_today && notifToday) {
      ...
      await Schedule.updateOne({ _id: schedule._id }, { $set: { notif_today: true } });
    }
    ...
    if (!schedule.notif_tomorrow && notifTomorrow) {
      ...
      await Schedule.updateOne({ _id: schedule._id }, { $set: { notif_tomorrow: true } });
    }
  }
  ...
  const all_read = await Notification.find({user_id: user_id, all_read: false});
